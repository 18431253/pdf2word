    ...
    for page in fitz_doc:
        ...
        for block in page.get_text("dict")["blocks"]:
            ...
            for line in block["lines"]:
                ...
                # Merge spans in same line into runs
                runs = []
                for span in spans:
                    t = _clean(span["text"])
                    if not t:
...
def blocks_to_docx(blocks_data: dict, out_path: str, ref_path: str = None):
    ...
    for page_data in blocks_data.get("pages", []):
        ...
        if page_data != blocks_data["pages"][-1]:
            doc.add_page_break()
...
def _copy_styles_from_ref(ref_doc, out_doc):
    ...
    existing_ids = {s.get(qn("w:styleId")) for s in out_styles_el.findall(qn("w:style"))}
